    ...
    public class LXOrbwalker
    {
        ...
        public static void Orbwalk(Vector3 goalPosition, Obj_AI_Base target)
        {
            ...
            if (!CanMove() || !IsAllowedToMove())
                return;
			if (MyHero.IsMelee() && target != null && Vector2.Distance(target.Position.To2D(), MyHero.Position.To2D()) < GetAutoAttackRange(MyHero, target) &&
                ...
                foreach (
                    var minion in
                        ...
                        let t = (int)(MyHero.AttackCastDelay * 1000) - 100 + Game.Ping / 2 +
								1000 * (int)Vector2.Distance(minion.Position.To2D(), MyHero.Position.To2D()) / (int)MyProjectileSpeed()
