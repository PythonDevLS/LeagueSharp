...
namespace RivenSharp
{
    class Riven
    {
        ...
        public static void doCombo(Obj_AI_Base target)
        {
            if (target == null)
                return;
            if (Vector2.Distance(target.Position.To2D(), Player.Position.To2D()) < 500)
            {
                sumItems.cast(SummonerItems.ItemIds.Ghostblade);
            }
			if (Vector2.Distance(target.Position.To2D(), Player.Position.To2D()) < 500 && (Player.Health / Player.MaxHealth) * 100 < 85)
            {
        ...
        public static void doHarasQ(Obj_AI_Base target)
        {
            ...
            {   
               // Player.IssueOrder(GameObjectOrder.AttackUnit, target);
                ...
                if (Q.IsReady() && dist > Player.AttackRange+Player.BoundingRadius/* && (getQJumpCount()*175 + target.BoundingRadius + 100) < dist*/)
                {
        ...
        public static void useESmart(Obj_AI_Base target)
        {
			...
			float dist = Vector2.Distance(target.Position.To2D(), Player.Position.To2D()); 
        ...
        public static void useRSmart(Obj_AI_Base target)
        {
            ...
            {
                R.Cast();
            } else if (canUseWindSlash() && target is Obj_AI_Hero && (!(E.IsReady() && Player.IsDashing()) || Vector2.Distance(target.Position.To2D(), Player.Position.To2D()) > 150))
            {   
